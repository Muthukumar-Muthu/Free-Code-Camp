...
const paraContainer = document.querySelector(".paragraph");
...
function formSubmitted(e) {
    ...
    if (value <= 0 || isNaN(value) || value > 9) {
...
function showRandom(number) {
    paraContainer.innerHTML = getPara(text[number]);
}
...
function showParaArray(value) {
    const randomArray = [],
        random = Math.floor(Math.random() * text.length);
    for (let i = random, position = 0;; i++) {
        if (i >= text.length) {
            i = 0;
        }
        randomArray[position++] = text[i];
        if (position == value) {
            break;
        }
    }

    const arrayOfP = randomArray
        .map((value) => {
            return getPara(value);
        })
        .join(" ");
    paraContainer.innerHTML = arrayOfP;
}